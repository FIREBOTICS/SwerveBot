...
public class SwerveModule extends SubsystemBase {
     ...
     * 
     * <p>
     * SwerveModule represents and handles a swerve module.
     * </p>
     * IDs: Front left - 1, front right - 2, back left - 3, back right - 4
     * (Front = intakeside)
     * 
     * @param moduleId           Module number, used to generate SPARK and CanCoder
     *                           IDs
    ...
    /**
     * Initializes the steer motor encoder to the value of the CANCoder, accounting
     * for the offset.
     */
    ...
    public Rotation2d getCanCoderAngle() {
        double unsignedAngle = (Units.rotationsToRadians((canCoder.getAbsolutePosition().refresh().getValue()))
    ...
    /**
     * Returns the current absolute angle of the module from the steeration motor
     * encoder.
     * 
    ...
    public double getRelativeVelocityMetersPerSecond(double thetaRad) {
        ...
        if (rel > 90.0 && rel < 270.0)
            rel *= -1.0;
        return getCurrentVelocityMetersPerSecond() * (rel / 90.0);
    }

    /**
     * Calculates the angle motor setpoint based on the desired angle and the
